...
'''This is My Code For FaceDetection+Recogination'''

def image_to_encoding(image_path, model):
    img1 = cv2.imread(image_path,1)
    resized = cv2.resize(img1, (96,96)) 
    img = resized[...,::-1]    
    img = np.around(np.transpose(img, (2,0,1))/255.0, decimals=12)
    x_train = np.array([img])
    embedding = model.predict_on_batch(x_train)
...
def who_is_it(roi_color, database, model):
    
    encoding = roi_embedding(roi_color,model)
...
# Resize the image with 300 width and the new height.
  image = cv2.resize(image, (300, new_height))
  return image   

def roi_embedding(roi_color,model):
